...
import { resetRepeaters, validateCallsign, validatePid, validateSsid, validateT1, type Repeater } from "../misc.js"
...
export class OutboundSession {
    ...
    public async disconnect(): Promise<void> {
        ...
        const disc: DISCFrame = new DISCFrame({
            ...
            sourceCallsign: this.tnc.sourceCallsign,
            sourceSsid: this.tnc.sourceSsid,
    ...
    public async receive(mode: 'concat' | 'payloadArray' | 'frameArray' = 'concat'): Promise< string | string[] | InboundFrame[] > {
        return new Promise< string | string[] | InboundFrame[] >((resolve) => {
            ...
            const handler = (frame: InboundFrame) => {
                if (
                    frame.destinationCallsign === this.tnc.sourceCallsign &&
                    frame.destinationSsid === this.tnc.sourceSsid &&
