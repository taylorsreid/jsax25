...
import { DISCFrame, IFrame, IncomingFrame, RRFrame, SABMEFrame, SABMFrame, type OutgoingConstructor } from "./index"
import { validatePid, type Repeater } from "./misc"

export class TNC extends Duplex {

    private _kissConnection!: Duplex // set in constructor via setter
    public get kissConnection(): Duplex {
        return this._kissConnection
    }
    public set kissConnection(value: Duplex) {
    ...
    public set retries(value: number) {
        if (value < 0 || !Number.isInteger(value)) {
            throw new Error(`The value for the number of retries must be a positive integer. Got ${value}`)
        }
        this._retries = value
    }
    ...
    constructor(args: {
        ...
        this.kissConnection.on('close', () => this.end())
